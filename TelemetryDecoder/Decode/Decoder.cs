...
namespace TelemetryDecoder.Decode
{
    abstract class Decoder
    {
        ...
        protected ReedSolo _reedSolo = new ReedSolo();
        protected Viterbi _viterbi = new Viterbi();
        ...
        private int ind_bt_in = 0; //Счетчик принимаемых байт.

        private int kol_pix;
        private int kol_all = 0;
        ...
        protected int ind_tk_in = 0;
        protected int ind_vit; //Счетчик после витерби.

        // Для поиска маркера ТК.
        protected byte mask_out_tk = 0x80;
        protected int Ind_mar_tk_bit = 0;
        protected bool Fl_err = true; // Флаг потери маркера ТК.
        protected int cnt_mark = 0; // Считаем биты маркера.

        // Для интерливинга.
        protected bool _isInterliving; // Состояние checkBox "Интерливинг".
        private bool[] interl_buf = new bool[Constants.DL_INTRL_BUF];

        protected int beg_mark_uw = -1; // Начало маркера интерливинга в битах.
        private int ind_mark_uw = 0; // Индекс 0-80.
        //Индекс по буф.интерливинга.
        private int ind_in_intrl = 0;
        private int ind_out_intrl = 0;

        private bool fl_fool_buf = false;
        private int ind_small = 0;
        private int ind_big = 0;
        private int ind_fr = 0;

        // Для проверки подлинности трансп. и парц. пакетов.
        protected int errs = 0; // Накапливаем ошибки здесь.
        private int tk_last = -1; // Номер ТК.
        private long tm_last = 0; // Последнее время.
        private int last_count_pac = -1; // Запоминаем счетчик пакетов.
        private int apid = -1; // Запоминаем последний апид.      
        private int timeShiftErrs = 0; // Накапливаем ошибки сдвига по времени.

        protected bool _isReedSolo; // Требуется ли декодирование Рида-Соломона.
        private byte Q_Value; // Фактор качества.
        private int dl_jpeg_in = -1; //Длина данных jpeg.
        //Индекс полосы при выводе. 
        private int Xt;
        private int Yt = 0;
        private int _lastXt = 0; //Последнее Xt

        // Для НРЗ.
        protected bool _isNrz; // Есть ли NRZ.
        protected bool last_bit_in = Convert.ToBoolean(0);
        ...
        public static bool IsSignalPhaseSync { get; set; } = false;
        public uint Kol_tk { get; set; } = 0; //Считает число транспортных кадров. 
        ...
        {
            Decoder.IsSignalPhaseSync = false;
            _sw.WriteLine("-------------------------------------------------");
            _sw.WriteLine("------------------------------------------");
                    ...
                    if (Ind_mar_tk_bit < 32) //зона маркера тк
                    {
                        ...
                        if (Ind_mar_tk_bit == 32)
                        {
                            if (cnt_mark == 32)
                            {
                                ...
                                Decoder.IsSignalPhaseSync = true;
                            }
        ...
        protected int Find_new_beg_zag(int k)
        {
            int i, j, n, kol;
